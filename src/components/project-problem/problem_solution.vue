<template>
  <div class="problem-solution">
    <el-container>
      <el-aside width="400px" style="padding-right:10px;">
        <el-card style="height:100%;box-sizing:border-box;" shadow="never">
          ...
          <el-table ref="problemTable" style="width: 100%;" :data="ProblemData" tooltip-effect="dark"
            highlight-current-row row-key="p_id" default-expand-all  @row-click="handleSelectproblemClick">

            <el-table-column type="index" label="序号" width="50" align="center"></el-table-column>
            <el-table-column prop="p_note" label="问题描述" width="200" align="center"></el-table-column>
      ...
      <el-main class="mainContent">
        ...
        <div class="tbar">
          ...
          <el-input size="small" @keyup.enter.native="refreshProblemSolutionData" placeholder="请输入方案名称" v-model="condition"
            clearable style="width:250px;">
            <el-button size="small" @click="refreshProblemSolutionData" slot="append" icon="el-icon-search">搜索</el-button>
          </el-input>
          <el-button type="primary" size="small" style="margin-left:10px;" @click="addNewPSShow">新增解决方案
          </el-button>
        ...
        <div class="gridTable">
          <zj-table :autoHeight='bottomDivShow' height='100%' ref="problemSolutionTable" v-loading="loading"
            ...
            <el-table-column prop="ps_note" label="方案描述" align="center" width="300" show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="ps_supply_user" label="提供者" align="center" width="100" show-overflow-tooltip></el-table-column>
            <el-table-column prop="ps_supply_date" label="提供时间" align="center" width="150" sortable show-overflow-tooltip>
            </el-table-column>
            <el-table-column prop="ps_state" label="状态" align="center" width="100" show-overflow-tooltip></el-table-column>
            ...
            <el-tab-pane label="方案详情" name="first">
              <div v-if="bottomDivShow">
                <div class="tbar">
                  ...
                  <el-button type="primary" size="small" style="margin-left:10px;" @click="addProductShow">新增方案详情
                  </el-button>
    ...
    <el-dialog v-if="addProblemSolutionVisible" v-dialogDrag width="450px" :title="addOrNot?'新增方案':'编辑方案'"
      ...
      <zj-form size="small" :newDataFlag='addProblemSolutionVisible' :model="problemSolutionModel" label-width="100px"
        ...
        <el-form-item label="方案详情">
          <el-input class="formItem" type="textarea" :rows="2" v-model="problemSolutionModel.ps_note" placeholder="方案详情">
          </el-input>
        </el-form-item>

        <el-form-item label="方案提供者">
          <el-input class="formItem"  v-model="problemSolutionModel.ps_supply_user" placeholder="方案提供者">
          </el-input>
        </el-form-item>

        <el-form-item label="提供时间">
          <!-- <el-input class="formItem"  v-model="problemSolutionModel.ps_supply_date" placeholder="提供时间">
          </el-input> -->
        ...
        <el-form-item label="状态">
          <el-input class="formItem" style="width:50%"  v-model="problemSolutionModel.ps_state" placeholder="模板说明">
          </el-input>
        </el-form-item>
    ...
    <!-- 新增物料需求 -->
    <el-dialog :width=" itemModelList.length? '1000px':'550px'" title="新增方案详情" :close-on-click-modal="false"
    ...
    <!-- 选择/修改物料 -->
    <el-dialog v-if="selectItemVisible" v-dialogDrag width="450px" :title="addOrNot?'新增方案详情':'编辑方案详情'"
...
export default {
  ...
  methods: {
    ...
    //选中的分类
    // handleSelectproblemClick(data) {
    //     this.refreshProblemData;
    //   this.selectProblem = data;
    //   this.searchProblemSolution();
    // },
...
//点击任务行显示右边编码详情
    ...
    //选择分类
    handleSelectproblemClick2(data) {
      this.problemSolutionModel.pc_no = data.pc_no;
      this.problemSolutionModel.pc_name = data.pc_name;
    ...
    addNewPSShow() {
      this.problemSolutionModel = {
        // pc_no: this.selectProblem.pc_no ? this.selectProblem.pc_no : "",
        // pc_name: this.selectProblem.pc_name ? this.selectProblem.pc_name : "",
        // pt_name: "",
        // pt_note: ""
    ...
    editTemplateShow(row) {
      ...
      this.problemSolutionModel.pc_name = this.renderFilter(
        this.problemSolutionModel.pc_no,
        this.classFilter
      );
      this.addOrNot = false;
      this.addProblemSolutionVisible = true;
    ...
    onSaveTemplateClick() {
      this.$refs.templateForm.validate(valid => {
        if (valid) {
          if (this.addOrNot) {
            this.z_post("api/project_template", this.problemSolutionModel)
              .then(res => {
                this.$message({
                  message: "新增成功!",
                  type: "success",
            ...
            this.problemSolutionModel.UpdateColumns = this.$refs.templateForm.UpdateColumns;
            if (this.problemSolutionModel.UpdateColumns) {
              this.z_put("api/project_template", this.problemSolutionModel)
                .then(res => {
                  this.$message({
                    message: "编辑成功!",
                    type: "success",
