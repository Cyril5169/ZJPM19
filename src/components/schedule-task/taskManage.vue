<template>
  <div class="taskManage">
    ...
    <!-- 新增任务、子任务、编辑任务-->
    <el-dialog width="700px" :title="addTaskText" :close-on-click-modal="false" :visible.sync="addTaskVisiable"
      top="15vh" @closed="refreshForm">
      <zj-form size="small" :model="taskModel" :newDataFlag='addTaskVisiable' label-width="100px" ref="taskForm"
        :rules="task_rules">
        <el-row>
          <el-col :span="11">
          ...
          <el-col :span="11">
            <el-form-item label="所属项目" prop="p_no">
              <el-select class="formItem" v-model="taskModel.p_no" placeholder="请选择所属项目"
                :disabled="taskModel.t_pid!=''">
                <el-option v-for="item in project_options" :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
            </el-form-item>
        ...
        <el-row>
          <el-col :span="11">
            <el-form-item label="计划开始时间" prop="t_early_startdate">
              <el-date-picker class="formItem" v-model="taskModel.t_early_startdate" type="date"
                format="yyyy 年 MM 月 dd 日" placeholder="请选择日期" :picker-options="pickerOptions">
              </el-date-picker>
            </el-form-item>
          </el-col>
          <el-col class="line" :span="1">&nbsp;</el-col>
          <el-col :span="11">
            <el-form-item label="计划结束时间" prop="t_last_enddate">
              <el-date-picker class="formItem" v-model="taskModel.t_last_enddate" type="date" format="yyyy 年 MM 月 dd 日"
                placeholder="请选择日期" :picker-options="pickerOptions">
              </el-date-picker>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-form-item label="备注">
          <el-input style="width:530px;" type="textarea" :rows="2" v-model="taskModel.t_note" placeholder="">
          </el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    selectLeafNode(inputList, outputList) {
      for (var i = 0; i < inputList.length; i++) {
        if (inputList[i].children.length == 0) {
          //只要没有子节点，就添加到输出的叶子节点集合中
        ...
        for (var j = 0; j < this.leafNode.length; j++) {
          ...
          if (res1.data.dic.length != 0) {
            for (var i = 0; i < res1.data.dic.length; i++) {
              if (
                mark == 1 &&
                res1.data.dic[i].dept_id &&
                res1.data.dic[i].tr_ismain
              ) {
                flag = true; //有主要执行部门
              }
              if (
                mark == 2 &&
                res1.data.dic[i].emp_id &&
                res1.data.dic[i].tr_ismain
              ) {
                flag = true; //有主要执行人
              }
            ...
            flag = false; //任务集中有记录没有设置任务执行者
          }
          if (!flag) {
            //判断当前任务是否满足排班或发布所需的执行者条件
            if (mark == 1) {
              this.$alert(
                "存在任务无执行部门或主要执行部门，不可排班",
                "提示",
                {
              ...
              this.$alert("存在任务无执行人或主要执行人，不可发布", "提示", {
                confirmButtonText: "好的",
                type: "warning"
              });
            }
